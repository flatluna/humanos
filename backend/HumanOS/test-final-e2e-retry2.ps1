...
try {
    ...
    $resp | ConvertTo-Json -Depth 10 | Set-Content -Path "$PSScriptRoot\final-e2e-retry2.json" -Encoding utf8
    "SUCCESS" | Set-Content -Path "$PSScriptRoot\final-e2e-retry2-status.txt"
} catch {
    $_.Exception.Message | Set-Content -Path "$PSScriptRoot\final-e2e-retry2-status.txt"
    if ($_.ErrorDetails) {
        $_.ErrorDetails.Message | Add-Content -Path "$PSScriptRoot\final-e2e-retry2-status.txt"
    }
}
