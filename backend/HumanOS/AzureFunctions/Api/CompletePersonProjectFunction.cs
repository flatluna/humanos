...
public sealed class CompletePersonProjectFunction
{
    private readonly ProjectService _projectService;

    public CompletePersonProjectFunction(ProjectService projectService)
    {
        _projectService = projectService;
    }

    [Function("CompletePersonProject")]
    public async Task<HttpResponseData> RunAsync(
        [HttpTrigger(
            AuthorizationLevel.Anonymous,
            "post",
            Route = "people/{personId:guid}/projects/{personProjectId:guid}/complete")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await _projectService.CompleteAsync(personProjectId, cancellationToken);
