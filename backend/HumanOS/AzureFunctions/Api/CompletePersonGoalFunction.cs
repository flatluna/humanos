...
public sealed class CompletePersonGoalFunction
{
    private readonly GoalService _goalService;

    public CompletePersonGoalFunction(GoalService goalService)
    {
        _goalService = goalService;
    }

    [Function("CompletePersonGoal")]
    public async Task<HttpResponseData> RunAsync(
        [HttpTrigger(
            AuthorizationLevel.Anonymous,
            "post",
            Route = "people/{personId:guid}/goals/{personGoalId:guid}/complete")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await _goalService.CompleteAsync(personGoalId, cancellationToken);
