...
try {
    ...
    $resp | ConvertTo-Json -Depth 10 | Set-Content -Path "$PSScriptRoot\spanish-retry-response.json" -Encoding utf8
    "SUCCESS" | Set-Content -Path "$PSScriptRoot\spanish-retry-status.txt"
} catch {
    $_.Exception.Message | Set-Content -Path "$PSScriptRoot\spanish-retry-status.txt"
    if ($_.ErrorDetails) {
        $_.ErrorDetails.Message | Add-Content -Path "$PSScriptRoot\spanish-retry-status.txt"
    }
}
